...
function isExiled(id) {
  return new Promise((resolve, reject) => {
    get("SELECT * FROM exiled_users WHERE id = ?", [id])
      .then(row => {
        if (row.id === id)
          return resolve({ status: true, oldNickname: row.oldNickname });
...
function getExileReason(id) {
  return new Promise((resolve, reject) => {
    get("SELECT reason FROM exiled_users WHERE id = ?", [id])
      .then(row => {
...
function exile(id, oldNickname, reason) {
  ...
  reason = reason.trim();
  return new Promise((resolve, reject) => {
    run("REPLACE INTO exiled_users (id, oldNickname, reason) VALUES (?,?,?)", [
      id,
      oldNickname,
      reason
    ])
...
function release(id) {
  return new Promise((resolve, reject) => {
    run("DELETE FROM exiled_users WHERE id = ?", [id])
      .then(() => {
        return resolve();
      })
      .catch(() => {
...
function tryTranslation(message) {
  return new Promise((resolve, reject) => {
    ...
    Translator.translate(message, "en", (error, translation) => {
      if (error)
        return resolve({ message: message, original: message, lang: langName });
      translation.detectedSourceLanguage = translation.detectedSourceLanguage
        .split("-", 1)
        .join("");
      return resolve({
        message: translation.translatedText,
        original: message,
        lang: ISO6391.getName(translation.detectedSourceLanguage)
      });
    });
  });
}
...
function execExile(message, member, channel, role) {
  ...
  tryChangeNickname(member, newNickname)
    .then(success => {
      if (success) {
        log(
          `${
            message.author.username
          } changed nickname of "${oldNickname}" to "${newNickname}"`
        );
      }
    ...
    .then(() => {
      log(
        `${message.author.username} assigned role "${
          role.name
        }" to "${newNickname}"`
      );
      const mention = member.toString();

      /* Try to send a message to the channel letting them know they are exileRoleName */
      return tryChannelSendMessage(
        channel,
        `${mention} ${Config.exileMessage}`
      );
    })
    .then(() => {
    ...
    .catch(error => {
      if (!(error instanceof BreakSignal)) {
        log(
          `Error assigning "${role.name}" to "${member.displayName}": ${error}`
        );
      }
    });
}
...
function execRelease(message, member, channel, role) {
  ...
  isExiled(member.id)
    ...
    .then(() => {
      log(
        `${message.author.username} removed role "${role.name}" from "${
          member.displayName
        }"`
      );
    })
...
Client.on("message", message => {
  ...
  tryTranslation(message.content).then(messageObj => {
    ...
    if (triggered) {
      ...
      log(msg);
      Client.guilds
        .get(Config.translation.notificationGuild)
        .channels.get(Config.translation.notificationChannel);
      Client.guilds
        .get(Config.translation.notificationGuild)
        .channels.get(Config.translation.notificationChannel)
        .send(msgPayload)
...
Client.on("message", message => {
  ...
  const channel = message.guild.channels.find(
    channel => channel.name === Config.exileChannelName
  );
  ...
  if (message.content.startsWith(`${Config.trigger}exile`)) {
    ...
    message.delete(Config.deleteAfter).catch(error => {
      log(error);
    });
      ...
      if (!isEnforcer(message.author.id)) {
        ...
        if (isEnforcer(member.id)) {
          ...
          return setTimeout(() => {
            ...
            log(
              `${message.author.username} has been released from the drunk tank`
            );
    ...
    message.content.startsWith(`${Config.trigger}release`) ||
    message.content.startsWith(`${Config.trigger}unexile`)
  ) {
    /* Set the message to delete */
    message.delete(Config.deleteAfter).catch(error => {
      log(error);
    ...
    if (!isEnforcer(message.author.id)) {
      getExileReason(message.author.id).then(reason => {
        if (reason.length !== 0) {
          ...
          return message.channel
            .send(
              `${mention} The message scrawled across your warrant states: ${reason}`
            )
            .catch(() => {});
        } else {
          return message.channel
            .send(`${mention} There is no warant out for you`)
            .catch(() => {});
        }
      });
    } else {
      ...
      getExileReason(target.id).then(reason => {
        if (reason.length !== 0) {
          return message.channel
            .send(`The warrant for ${target} states: ${reason}`)
            .catch(() => {});
        }
      });
    }
...
Client.on("guildMemberAdd", member => {
  ...
  isExiled(member.id)
    ...
    .then(() => {
      ...
      return tryChannelSendMessage(
        channel,
        `${mention} ${Config.exileEvadeMessage}`
      );
    })
    .catch(error => {
      if (!(error instanceof BreakSignal)) {
...
Client.login(Config.token).catch(err => {
  log(
    "There was an error logging into Discord... please check your token and try again"
  );
  log(err.toString());
});
